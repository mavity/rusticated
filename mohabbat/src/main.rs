...
async fn build_project(project_dir: &str, self_path: &str) -> anyhow::Result<String> {
    ...
    // Detect whether this is a Go project (go.mod present) or a Rust project (Cargo.toml).
    let go_mod_path = format!(
        "{}/go.mod",
    ...
    let has_go_mod = read_all(&go_mod_path).await.is_ok();
    if has_go_mod {
        return build_go_project(project_dir, &workspace_root).await;
    }
    build_cargo_project(&cargo_toml_path, &workspace_root).await
}
...
async fn build_go_project(project_dir: &str, workspace_root: &str) -> anyhow::Result<String> {
    ...
    out_print(&format!("🍆 Building Go package: {}\n", project_name)).await;

    // Ensure overlay.json exists (generated by cargo run -p prebuild).
    let overlay_path = format!("{}/target/overlay.json", workspace_root);
    ...
    let status = child
        .wait()
        .await
        .map_err(|e| anyhow::anyhow!("go build wait failed: {}", e))?;
