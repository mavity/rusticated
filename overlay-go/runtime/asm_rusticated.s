...
TEXT runtime·resume_asm(SB),NOSPLIT,$0
    CALL wasm_pc_f_loop(SB)
    RET

TEXT runtime·getsp_asm(SB),NOSPLIT,$0
    MOVD SP, ret+0(FP)
    RET
...
TEXT runtime·rusticated_pause(SB), NOSPLIT, $0-8
        MOVD newsp+0(FP), SP
        I32Const $1
        Set PAUSE
        RETUNWIND
...
TEXT rusticated_entry(SB),NOSPLIT,$0
    CALL runtime·rusticated_entry_go(SB)
    JMP  wasm_pc_f_loop(SB)

TEXT runtime·rusticated_pause_asm(SB),NOSPLIT,$0
    CALL wasm_pc_f_loop(SB)
    I32Const $0
    Return
