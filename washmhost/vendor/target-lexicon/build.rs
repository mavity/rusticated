//! build.rs for the rusticated shim of target-lexicon.
//!
//! Identical to the upstream target-lexicon 0.12.16 build.rs except that it
//! translates custom `*-rusticated` target names to their standard equivalents
//! before parsing.  This allows wasmtime (and any other user of target-lexicon)
//! to compile without panicking when the TARGET env var is set to a custom
//! rusticated target triple.
...
/// Translate a custom rusticated target name to the standard triple that it is
/// derived from.  The rusticated targets are standard targets with a different
/// `std` crate substituted; all other properties (arch, vendor, OS, ABI) are
/// identical to the base triple.
fn translate_rusticated(target: &str) -> &str {
    match target {
        "aarch64-windows-rusticated" => "aarch64-pc-windows-msvc",
        "x86_64-windows-rusticated"  => "x86_64-pc-windows-msvc",
        "aarch64-linux-rusticated"   => "aarch64-unknown-linux-gnu",
        "x86_64-linux-rusticated"    => "x86_64-unknown-linux-gnu",
        "wasm32-rusticated"          => "wasm32-unknown-unknown",
        other                        => other,
    }
}
...
fn main() {
    ...
    let raw_target = env::var("TARGET").expect("The TARGET environment variable must be set");

    // Translate any custom rusticated target name to the equivalent standard
    // triple before handing it to the parser.
    let target = translate_rusticated(&raw_target).to_string();
